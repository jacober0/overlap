...
describe('redaktioneller Rezeptbatch 78', () => {
  it('ergänzt zwölf eigenständige und inhaltlich veröffentlichungsfähige Originalrezepte', () => {
    expect(editorialRecipes.slice(-12).map(recipe => recipe.externalId)).toEqual(batch78Ids)
    expect(editorialRecipes.slice(-12).map(recipe => recipe.title)).toEqual(batch78Titles)
    expect(editorialRecipes).toHaveLength(321)
