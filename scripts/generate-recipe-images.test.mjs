...
describe('recipe image manifest and prompt contract', () => {
  it('defines a dish-specific prompt for every published recipe in the app catalog', () => {
    const manifestIds = new Set(recipeImageManifest.map(({ id }) => id))
    // Every recipe the app surfaces must have a matching generation prompt.
    for (const recipe of recipes) {
      expect(manifestIds).toContain(recipe.id)
    }
    // No duplicate ids or dishes, and every prompt must be substantive.
    expect(new Set(recipeImageManifest.map(({ id }) => id)).size).toBe(recipeImageManifest.length)
    expect(new Set(recipeImageManifest.map(({ dish }) => dish)).size).toBe(recipeImageManifest.length)
